...
function mapCardToStoryPoints(estimate: string): { numeric: number | null; label: string } {
...
function buildCommentText(input: SyncEstimateInput): string {
  const lines: string[] = [
    '[Planning poker]',
...
export async function syncEstimateToJira(
  firebaseUid: string,
  input: SyncEstimateInput,
): Promise<{ firestoreUpdated: boolean }> {
  const { cloudId, issue } = await getIssueWithCloudId(
    firebaseUid,
    input.jiraIssueKey,
    input.jiraSiteUrl,
  );
  ...
  if (includeAuditIssueProperty()) {
    ...
    if (prev && typeof prev === 'object' && prev !== null && 'entries' in prev) {
      const e = (prev as { entries: unknown }).entries;
      if (Array.isArray(e)) {
        entries = [...e];
      }
    }
